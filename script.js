(() => {
  ...
  document.getElementById("eduCard").innerHTML = `
    <div class="edu-main">
      ...
      <p class="edu-org">${EDUCATION.org}</p>
      <p class="edu-meta">${EDUCATION.meta}</p>
    </div>
    <div class="edu-gpa">
      <span class="gpa-num">${EDUCATION.gpa}</span>
      <span class="gpa-label">GPA / 4.0</span>
    </div>
  `;

  /* ---------- render: skills ---------- */
  document.getElementById("skillsGrid").innerHTML = SKILLS.map(g => `
  ...
  document.getElementById("projectsGrid").innerHTML = PROJECTS.map(p => `
    <article class="project-card">
      ...
      <div class="project-block">
        ...
        <ul class="project-steps">${p.implementation.map(s => `<li>${s}</li>`).join("")}</ul>
      </div>
  ...
  /* ---------- render: certifications (image visible on the card) ---------- */
  ...
  certGrid.innerHTML = CERTIFICATIONS.map((c, i) => `
    <button class="cert-card" data-cert="${i}" type="button">
      <div class="cert-thumb-wrap">
        ${c.image ? `<img class="cert-thumb" src="${c.image}" alt="${c.title} certificate" loading="lazy">` : ""}
      </div>
      <div class="cert-card-body">
        <h3>${c.title}</h3>
        <p class="cert-issuer">${c.issuer}</p>
        <p class="cert-date">${c.date}</p>
        <p class="cert-view">View full certificate →</p>
      </div>
    </button>
  `).join("");
  ...
  /* ---------- render: footer ---------- */
  document.getElementById("footerBlurb").textContent = FOOTER_BLURB;

  document.getElementById("footerNav").innerHTML =
    NAV_SECTIONS.map(([id, label]) => `<a href="#${id}">${label}</a>`).join("");

  /* Platform marks, drawn as simple monograms so no external logo files are needed. */
  const SOCIAL_ICONS = {
    mostaql: '<circle cx="12" cy="12" r="10"/><path d="M7.4 15.2V9.6l2.4 3.1 2.2-3.1 2.2 3.1 2.4-3.1v5.6" stroke-width="1.5"/>',
    khamsat: '<circle cx="12" cy="12" r="10"/><path d="M14.4 7.8H9.9l-.5 3.4c.6-.4 1.3-.6 2-.6a3 3 0 1 1 0 6 3 3 0 0 1-2.7-1.6" stroke-width="1.5"/>',
    upwork:  '<circle cx="12" cy="12" r="10"/><path d="M6.6 9.2v3.4a2.5 2.5 0 0 0 5 0V9.2m0 2.2c.7 1.8 1.6 3.4 3 3.4a2.4 2.4 0 1 0 0-4.8c-1.6 0-2.5 1.5-3 3.2" stroke-width="1.5"/>',
    linkedin: '<rect x="3" y="3" width="18" height="18" rx="2.5"/><path d="M7.2 10.4v6.2M7.2 7.6v.1M11 16.6v-6.2m0 1.6c.5-1 1.4-1.6 2.5-1.6 1.5 0 2.5 1 2.5 2.8v3.4" stroke-width="1.5"/>',
    github: '<path d="M9.3 20.4c-4 1.2-4-2.1-5.6-2.5m11.2 5v-3.3c0-.9.1-1.3-.4-1.8 2.3-.3 4.6-1.1 4.6-5a3.9 3.9 0 0 0-1.1-2.7 3.6 3.6 0 0 0-.1-2.7s-.9-.3-2.9 1.1a10 10 0 0 0-5.2 0C7.8 5.1 6.9 5.4 6.9 5.4a3.6 3.6 0 0 0-.1 2.7 3.9 3.9 0 0 0-1.1 2.7c0 3.9 2.3 4.7 4.6 5-.3.3-.5.8-.4 1.3v3.4"/>',
  };
  const visibleSocials = SOCIALS.filter(s => s.url && s.url !== "#");
  document.getElementById("footerSocials").innerHTML = visibleSocials.map(s => `
    <li>
      <a href="${s.url}" target="_blank" rel="noopener noreferrer" title="${s.label}" aria-label="${s.label}">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">${SOCIAL_ICONS[s.key] || ""}</svg>
      </a>
    </li>
  `).join("");
  ...
  if (PROFILE.email && !PROFILE.email.includes("PLACEHOLDER")) {
    footerContact.push({ icon: "mail", label: PROFILE.email, href: `mailto:${PROFILE.email}` });
  }
  document.getElementById("footerContact").innerHTML = footerContact.map(c => `
    <li>
      <span class="c-icon">${icon(c.icon, 16)}</span>
      ${c.href ? `<a href="${c.href}">${c.label}</a>` : `<span>${c.label}</span>`}
    </li>
  `).join("");
